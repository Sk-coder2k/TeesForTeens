...
export default function AdminProductsPage() {
    ...
    if (editingId) {
      ...
      updateProduct(editingId, {
        ...
        price: newProduct.price.startsWith("₹")
          ? newProduct.price.replace("₹", "")
          : newProduct.price,
        originalPrice: Number(newProduct.originalPrice) || 0,
        stock: parseInt(newProduct.stock.toString()) || 0,
        status:
          parseInt(newProduct.stock.toString()) <= 0
            ? "Out of Stock"
            : parseInt(newProduct.stock.toString()) <= 10
              ? "Low Stock"
              : "Active",
  ...
  const handleEditInit = (product: any) => {
    ...
    sizes.forEach((s: string) => {
      sizeStock[s] = product.sizeStock?.[s] ?? 0;
    });
    const colorStock: Record<string, number> = {};
    colors.forEach((c: string) => {
      colorStock[c] = product.colorStock?.[c] ?? 0;
    });
    setNewProduct({
      name: product.name,
  ...
  const lowStockProducts = products.filter(
    (p: any) => p.stock > 0 && p.stock <= LOW_STOCK_THRESHOLD,
  );
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-700 mt-0.5">
                  ...
                  running low (≤{LOW_STOCK_THRESHOLD} units):
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-gray-100 rounded-lg overflow-hidden shrink-0">
                          <img
                            src={
                              product.images?.[0] ||
                              `https://images.unsplash.com/photo-${1500000000000 + String(product.id).charCodeAt(0) * 500}?auto=format&fit=crop&q=80&w=100`
                            }
                            alt={product.name}
          ...
          <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
            <div>
              Showing {filteredProducts.length > 0 ? 1 : 0} to{" "}
              {filteredProducts.length} of {filteredProducts.length} entries
            </div>
            <div className="flex gap-1">
              <button className="px-3 py-1 border border-gray-200 rounded disabled:opacity-50">
                Prev
              </button>
              <button className="px-3 py-1 border border-mint-600 bg-mint-50 text-mint-700 rounded font-medium">
                1
              </button>
              <button className="px-3 py-1 border border-gray-200 hover:bg-gray-50 rounded">
                2
              </button>
              <button className="px-3 py-1 border border-gray-200 hover:bg-gray-50 rounded">
                3
            ...
            <div className="space-y-4 text-black">
              ...
              <div>
                ...
                {newProduct.images.length > 0 && (
                  <div className="flex gap-3 overflow-x-auto pb-2 mb-3 snap-x">
                    {newProduct.images.map((img, idx) => (
                      <div
                        key={idx}
                        className="relative group shrink-0 w-24 h-24 rounded-lg overflow-hidden border border-gray-200 snap-center"
                      >
                        <img
                          src={img}
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-1">
                          ...
                          <div className="flex justify-between gap-1">
                            ...
                            {idx === 0 && (
                              <span className="text-[10px] font-bold text-white bg-mint-600 px-1.5 py-0.5 rounded shadow-sm">
                                Main
                              </span>
                            )}
                            <button
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-1 pl-2">
                    Upload multiple images one by one. First image is the main
                    display.
                  </p>
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-mint-500"
                  />
                  <button
                    onClick={() => {
                      ...
                      if (cat && !allCategories.includes(cat)) {
                        ...
                        setNewProduct((p) => ({ ...p, category: cat }));
                        setNewCategoryInput("");
                      }
                    }}
                    className="px-3 py-1.5 bg-[#29bc89] text-white rounded-lg text-xs font-bold hover:bg-[#23a172]"
                  >
                    + Add
                  </button>
                </div>
                ...
                if (op > sp && sp > 0) {
                  ...
                  return (
                    <div className="flex items-center gap-2 px-1 -mt-1">
                      <span className="bg-[#FF6B6B] text-white text-[10px] font-bold px-2 py-[3px] rounded-full">
                        -{pct}%
                      </span>
                      <span className="text-xs text-gray-500">
                        Customers will see{" "}
                        <span className="font-bold text-gray-900">₹{sp}</span>{" "}
                        <span className="line-through">₹{op}</span>
                      </span>
                    </div>
                  );
                }
              ...
              <div className="pt-4 border-t border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <span className="text-xs text-mint-600 font-bold uppercase mb-2 block tracking-wider">
                      Stock by Size
                    </span>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {newProduct.sizes.map((size) => (
                        <div key={size} className="flex items-center gap-1">
                          <label className="text-xs font-bold text-gray-500 w-5 shrink-0">
                            {size}
                          </label>
                          <input
                            type="number"
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (
                            newSizeInput &&
                            !newProduct.sizes.includes(newSizeInput)
                          ) {
                            setNewProduct((prev) => ({
                              ...prev,
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          if (
                            newColorInput &&
                            !newProduct.colors.includes(newColorInput)
                          ) {
                            setNewProduct((prev) => ({
                              ...prev,
