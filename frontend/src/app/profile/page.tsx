...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"orders" | "info" | "settings">(
    "orders",
  );
  ...
  useEffect(() => {
    const fetchMyOrders = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/users/myorders`, {
        ...
        const normalized = data.map((o: any) => ({
          ...
          shipping:
            o.shippingPrice === 0 ? "Free" : o.shippingPrice?.toString() || "0",
    ...
    const fetchMyReviews = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setMyReviews(
            data.map((r: any) => ({ product: r.product, order: r.order })),
          );
        }
      } catch (e) {}
    };
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMyReviews((prev) => [
          ...prev,
          { product: reviewModal.productId, order: reviewModal.orderId },
        ]);
        setReviewModal(null);
        setReviewRating(5);
        setReviewTitle("");
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
                      ...
                      <div className="space-y-4">
                        {orders
                          ...
                          .map((order) => (
                            <div
                              key={order.id}
                              className="border border-gray-200 rounded-xl p-5 hover:border-mint-300 transition-colors"
                            >
                                ...
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    <p className="font-bold text-gray-900 flex items-center gap-2 flex-wrap">
                                      {order.displayId}
                                      <span
                                        className={`text-xs px-2 py-1 rounded-md font-bold ${statusStyle(order.status)}`}
                                      >
                                        {order.status}
                                      </span>
                                    </p>
              ...
              {activeTab === "orders" && selectedOrder && (
                <div className="animate-in fade-in duration-300">
                  ...
                  >
                    <ChevronRight className="w-4 h-4 rotate-180 mr-1" /> Back to
                    Orders
                  </button>
                      ...
                      {selectedOrder.status === "Cancelled" ? (
                        <div className="flex flex-col items-center justify-center py-8 gap-3">
                          ...
                          <p className="text-sm text-gray-500">
                            This order has been cancelled and stock has been
                            restored.
                          </p>
                        </div>
                      ) : (
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-4 md:gap-0 relative z-10">
                            {[
                              ...
                              return (
                                <div
                                  key={stage}
                                  className="flex md:flex-col items-center gap-4 md:gap-3 relative"
                                >
                                  {index > 0 && isCompleted && (
                                    <div className="absolute top-6 right-1/2 w-full h-1 bg-mint-500 hidden md:block z-0" />
                                  )}
                                  <div
                                    className={`relative z-10 w-12 h-12 rounded-full flex items-center justify-center border-4 border-white shadow-sm transition-all duration-500 shrink-0 ${isCompleted ? "bg-mint-500 text-white scale-110" : "bg-gray-100 text-gray-400"}`}
                                  >
                                    <Package
                                  ...
                                  <div className="md:text-center">
                                    <p
                                      className={`font-bold text-sm ${isCompleted ? "text-gray-900" : "text-gray-400"}`}
                                    >
                                      {stage}
                                    </p>
                                    {isCurrent && (
                    ...
                    <div className="lg:col-span-2 space-y-8">
                                ...
                                return (
                                  ...
                                  >
                                    <img
                                      src={item.image}
                                      alt={item.name}
                                      className="w-16 h-16 object-cover rounded-lg bg-gray-100 border border-gray-200 flex-shrink-0"
                                    />
                      ...
                      <div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 grid grid-cols-2 gap-3 text-sm">
                          ...
                          <div className="font-bold text-right text-base text-mint-600">
                            {selectedOrder.total}
                          </div>
                        </div>
                      </div>
                      ...
                      {(selectedOrder.status === "Processing" ||
                        ...
                        <div className="bg-red-50 rounded-xl p-5 border border-red-100 text-sm">
                          ...
                          <p className="text-red-600/80 mb-4">
                            You can cancel before it ships for a full refund.
                          </p>
                          <button
                            onClick={() =>
                  ...
                  <form onSubmit={handleSaveInfo} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="md:col-span-2">
                        ...
                        />
                        <p className="text-xs text-gray-400 mt-1">
                          This is auto-filled in checkout when you tick "Save as
                          default"
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                      ...
                      >
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-gray-50 px-4 py-2 border border-gray-300 rounded-lg focus:ring-mint-500 focus:bg-white"
                            required
                          />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="border border-red-100 bg-red-50 rounded-xl p-5">
                      ...
                      <p className="text-sm text-red-600/80 mb-4">
                        Permanently delete your account and all associated order
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl z-10 p-6 m-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star
                      className={`w-8 h-8 transition-colors ${star <= (reviewHover || reviewRating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                    />
                  </button>
                ))}
                <span className="ml-2 text-sm font-bold text-gray-700 self-center">
                  {reviewRating}/5
                </span>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-mint-500 focus:outline-none resize-none text-black"
              />
            </div>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-mint-600 text-white rounded-xl font-bold hover:bg-mint-500 transition-colors disabled:opacity-50"
              >
