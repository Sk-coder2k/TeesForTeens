...
export default function ProductDetail({
  ...
  const RELATED = products
    .filter((p) => p.id !== slug && p.id !== undefined)
    .slice(0, 4);
    ...
    const fetchReviews = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api"}/reviews/product/${slug}`,
        );
        if (res.ok) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col">
            <span className="text-mint-600 font-bold uppercase tracking-wider text-sm mb-2">
              {PRODUCT.category}
            </span>
            ...
            <div className="flex items-center mb-6">
              <div className="flex text-yellow-400">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                    className={`h-5 w-5 ${star <= Math.round(PRODUCT.rating) ? "fill-current" : "text-gray-300"}`}
                  />
                ))}
              </div>
                ...
                Number(PRODUCT.price) > 0 && (
                  <>
                    ...
                    <span className="text-xs font-bold text-mint-800 bg-mint-100 px-2.5 py-1 rounded-full">
                      {Math.round(
                        (1 - Number(PRODUCT.price) / PRODUCT.originalPrice) *
                          100,
                      )}
                      % OFF
                    </span>
            ...
            <div className="mb-8">
              ...
              <div className="flex gap-3">
                {(PRODUCT.colors || []).map((color: string) => {
                  const isSoldOut =
                    typeof PRODUCT.colorStock?.[color] === "number" &&
            ...
            <div className="flex gap-3 mb-10">
              ...
              <button
                ...
                className={`w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 rounded-xl border flex items-center justify-center transition-colors hover-lift ${isWished ? "border-mint-500 bg-mint-50" : "border-gray-200 bg-white hover:border-gray-300"}`}
              >
                <Heart
              ...
              <div className="flex items-center gap-4">
                <div className="flex text-yellow-400">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star
                      key={i}
                      className={`w-6 h-6 ${i <= Math.round(avgRating) ? "fill-current" : "text-gray-300"}`}
                    />
                  ))}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {reviews.map((review: any) => {
                ...
                return (
                  <div
                    key={review._id}
                    className="bg-white p-6 rounded-2xl shadow-sm border border-mint-100"
                  >
